...
def read_from_file():
    ...
    try:
        ...
        print(start_date.toString('dd-MM-yy'), calc_date.toString('dd-MM-yy'), description)
        ...
        print("FFFFFFF", delta_days_left, delta_days_right, days_total)
...
def on_click_calendar():
    ...
    calc_date = form.calendarWidget.selectedDate()  
    delta_days = start_date.daysTo(calc_date) #расчет кол-ва дней
    print(delta_days)
...
form.label.setText("Трекер события от %s " % start_date.toString('dd-MM-yy'))
on_click_calendar()
